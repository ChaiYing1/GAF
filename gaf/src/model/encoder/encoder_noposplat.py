...
class EncoderNoPoSplat(Encoder[EncoderNoPoSplatCfg]): #
    backbone: nn.Module # "ViTLarge_BaseDecoder
    gaussian_adapter: GaussianAdapter

    def __init__(self, cfg: EncoderNoPoSplatCfg) -> None:
        super().__init__(cfg)

        self.backbone = get_backbone(cfg.backbone, 3) # croco
    ...
    def set_center_head(self, output_mode, head_type, landscape_only, depth_mode, conf_mode):
        ...
        # predict action variations heads
        self.downstream_head_act1 = head_factory(head_type, output_mode, self.backbone, has_conf=bool(conf_mode))
        self.downstream_head_act2 = head_factory(head_type, output_mode, self.backbone, has_conf=bool(conf_mode))

        # magic wrapper
        self.head1 = transpose_to_landscape(self.downstream_head1, activate=landscape_only)
        self.head2 = transpose_to_landscape(self.downstream_head2, activate=landscape_only)

        self.head3 = transpose_to_landscape(self.downstream_head_act1, activate=landscape_only)
        self.head4 = transpose_to_landscape(self.downstream_head_act2, activate=landscape_only)
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            res3 = self._downstream_head(3, [tok.float() for tok in dec1], shape1)
            res4 = self._downstream_head(4, [tok.float() for tok in dec2], shape2)
        ...
        pts3d_act1 = res3['pts3d'] 
        pts3d_act1 = rearrange(pts3d_act1, "b h w d -> b (h w) d") 
        pts3d_act2 = res4['pts3d']
        pts3d_act2 = rearrange(pts3d_act2, "b h w d -> b (h w) d")
        pts_act_all = torch.stack((pts3d_act1, pts3d_act2), dim=1)
        pts_act_all = rearrange(pts_act_all, "b v n d -> b (v n) d")
        ...
        gaussians_next_means = rearrange(
                gaussians.means.clone(),
                "b v r srf spp xyz -> b (v r srf spp) xyz",
            )
        gaussians_next_means +=  pts_act_all
