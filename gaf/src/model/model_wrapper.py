...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.distiller is not None:
            visualization_dump = {}
        gaussians, gaussians_next = self.encoder(batch["context"], self.global_step, visualization_dump=visualization_dump)
        # print(gaussians.means)
        # print(gaussians_next.means)
        # torch.save(gaussians, 'gaussians.pt')
        ...
        # render gaussians_next
        output_next = self.decoder.forward(
            gaussians_next,
            batch["target"]["extrinsics"],
            batch["target"]["intrinsics"],
            batch["target"]["near"],
        ...
        # Get the next frame image; the dataset should also be updated accordingly
        target_gt_next = batch["target"]["image_next"] 
        ...
        psnr_probabilistic = compute_psnr(
            rearrange(target_gt_next, "b v c h w -> (b v) c h w"),
            rearrange(output_next.color, "b v c h w -> (b v) c h w"),
        )
        self.log("train/psnr_next_probabilistic", psnr_probabilistic.mean())

        # Compute and log loss.
        loss_now = 0
        for loss_fn in self.losses:
            loss = loss_fn.forward(output, target_gt, gaussians, self.global_step)
            loss_now = loss_now + loss
        self.log(f"loss/now:", loss_now)
        
        # Add the loss for predicting the next frame, the image rendered from gaussians_next and  target_gt_next
        loss_next = 0
        for loss_fn in self.losses:
            loss = loss_fn.forward(output_next, target_gt_next, gaussians_next, self.global_step)
            loss_next = loss_next + loss
        self.log(f"loss/next:", loss_next)
        

        # Add constraints on the point cloud boundaries
        boundary = 8
        loss_gaussians = 0       
        mask = (gaussians.means.abs() > boundary).any(dim=-1).float() 
        penalty_loss = (gaussians.means[mask.bool()]) ** 2
        loss_gaussians = penalty_loss.sum()
        if loss_gaussians > 0:
            loss_gaussians = torch.log(loss_gaussians) / 10
        self.log(f"loss/gaussians:", loss_gaussians)
        
        loss_gaussians_next = 0
        mask_next = (gaussians_next.means.abs() > boundary).any(dim=-1).float() 
        penalty_loss_next = (gaussians_next.means[mask_next.bool()]) ** 2
        loss_gaussians_next = penalty_loss_next.sum()
        if loss_gaussians_next > 0:
            loss_gaussians_next = torch.log(loss_gaussians_next) / 10
        self.log(f"loss/gaussians_next:", loss_gaussians_next)
        
        total_loss = loss_now + loss_next + loss_gaussians + loss_gaussians_next
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Render Next Gaussians.
        with self.benchmarker.time("encoder"):
            gaussians, gaussians_next = self.encoder(
                batch["context"],
                self.global_step,
            )
        ...
        if self.test_cfg.align_pose:
            output = self.test_step_align(batch, gaussians_next)
        else:
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        rgb_next_pred = output_next.color[0]
        depth_next_pred = vis_depth_map(output_next.depth[0])
        ...
        gaussian_next_means = visualization_dump["depth"][0].squeeze()
        if gaussian_next_means.shape[-1] == 3:
            gaussian_next_means = gaussian_next_means.mean(dim=-1)
        ...
        rgb_next_gt = batch["target"]["image_next"][0]
        psnr_next = compute_psnr(rgb_next_gt, rgb_next_pred).mean()
        self.log(f"val/psnr_next", psnr_next)
        lpips_next = compute_lpips(rgb_next_gt, rgb_next_pred).mean()
        self.log(f"val/lpips_next", lpips_next)
        ssim_next = compute_ssim(rgb_next_gt, rgb_next_pred).mean()
        self.log(f"val/ssim_next", ssim_next)
        ...
        comparison_next = hcat(
            add_label(vcat(*context), "Context"),
            add_label(vcat(*rgb_next_gt), "Target (Ground Truth)"),
            add_label(vcat(*rgb_next_pred), "Target (Prediction)"),
            add_label(vcat(*depth_next_pred), "Depth (Prediction)"),
        )
        ...
        self.logger.log_image(
            "comparison_next",
            [prep_image(add_border(comparison_next))],
            step=self.global_step,
            caption=batch["scene"],
        )
        ...
        self.logger.log_image(
            "projection_next",
            [prep_image(add_border(projections_next))],
            step=self.global_step,
        )
