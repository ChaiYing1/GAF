...
class Transformer(nn.Module):
    def __init__(self,
        ...
        )  # fixed sin-cos embedding: (1, 1, 1, num_patches + 1, self.embed_dim)
        ################################################################################################################
    ...
    def forward(
        ...
        pcd_embeds = []
        for i in range(self.num_views):
            img_embeds.append(self.embed_layer_rgb[i](imgs[:, i])) # [b * t, num_patches, self.embed_dim/2]
            if self.add_pcd:
                pcd_embeds.append(self.embed_layer_pcd[i](pcds[:, i])) # [b * t, num_patches, self.embed_dim/2]

        if self.add_pcd:
            img_embeds = torch.stack(img_embeds, dim=1).view(b, t, v, self.num_patches, -1) # [b, t, v, num_patches, self.embed_dim/2]
            pcd_embeds = torch.stack(pcd_embeds, dim=1).view(b, t, v, self.num_patches, -1) # [b, t, v, num_patches, self.embed_dim/2]
            embeds = torch.cat([img_embeds, pcd_embeds], dim=-1) # 形状为 [b, t, v, num_patches, self.embed_dim]
