    ...
    def forward(self, obs_data, action=None, store_imgs=False):
        ...
        if action is not None:
            ...
            if self.pred_actions:
                model_ret = self.net(obs_data, actions=self.normalizer.normalize(noisy_actions)[..., :-1]) # 训练的时候不会保存照片
            else:
        ...
        # inference
            ...
            with torch.no_grad():
                ...
                for k in range(self.inference_num_diffusion_iters - 1, -2, -1):
                    ...
                    ####################################################################################################
                    for i in range(self.pred_horizon):
                        if len(pcds_w[i]) < 3 and self.pred_actions:  
                            noisy_action_new[0, i, :-1] = pred_actions[0, i, :]
                            continue
                        T_w_w = get_rigid_transform(pcds_w[i],preds_w[i]) 
