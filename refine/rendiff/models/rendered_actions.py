...
class RenActRep(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, data): # T_x_y denotes a transformation from coordinate frame x to coordinate frame y
        ...
        ################################################################################################################
        return {'renders': images, # (len(self.camera_names), bs, self.pred_horizon, 4, H, W) 4=RGBA 
                'masks': masks, # (len(self.camera_names), bs, self.pred_horizon, H, W)
                'labels': labels, # (len(self.camera_names), bs, self.pred_horizon, 3, H, W) 3=xyz
                'points': points,  # (len(self.camera_names), bs, self.pred_horizon, 3, H, W) 3=xyz
                'rgbs': rgbs, # (len(self.camera_names), bs, 3, H, W) 3=RGB
                'rgbs_past': rgbs_past # (len(self.camera_names), bs, self.action_history, 3, H, W)
                }
